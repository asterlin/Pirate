$(document).ready(function () {
    var div = document.createElement('div'),
        ...
        rainArr = [1000],
        rainSpeed = 50;
        canvas.classList.add("loadingsRBgWeather");

    // initialize
    function init() {
        document.getElementsByClassName('loadingsYItembg')[0].appendChild(div);
    ...
    function create_rain() {
        ...
        for (var j = 0; j < 500; j++) {
            rainArr[j].x = Math.floor((Math.random() * 3000) - 9);
            rainArr[j].y = Math.floor((Math.random() * 3080 ) - 9);
            rainArr[j].z = Math.floor((Math.random() * 2) + 2);
    ...
    function UpdatePosition() {
        var bodyWidth = 2*document.getElementsByClassName('loadingsYItembg')[0].offsetWidth,
            bodyHeight = 2*document.getElementsByClassName('loadingsYItembg')[0].offsetHeight;
    ...
    function rain() {
        for (var i = 0; i < 500; i++) {
            ...
            ctx.fillStyle = grd;
            ctx.fillRect(rainArr[i].x, rainArr[i].y, rainArr[i].z, 100);
        }
    }
